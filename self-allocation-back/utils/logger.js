const winston = require('winston');
require('winston-daily-rotate-file');
const path = require('path');
const fs = require('fs');

// 控制台颜色定义
const colors = {
    reset: '\x1b[0m',
    bold: '\x1b[1m',
    red: '\x1b[31m',
    green: '\x1b[32m',
    yellow: '\x1b[33m',
    magenta: '\x1b[35m',
    brightRed: '\x1b[91m',
    brightGreen: '\x1b[92m',
    brightMagenta: '\x1b[95m'
};

// 创建日志目录
const logDir = path.join(process.cwd(), 'logs');
if (!fs.existsSync(logDir)) {
    fs.mkdirSync(logDir, { recursive: true });
}

// 创建日志格式
const logFormat = winston.format.printf(({ timestamp, level, message, module, pid }) => {
    const icon = level === 'error' ? '❌' : level === 'warn' ? '⚠️' : '✅';
    const moduleInfo = module ? `[${module}]` : '';
    const processInfo = `[PID:${pid || process.pid}]`;
    
    // 修复this.console的问题，改用传入的参数判断是否为控制台输出
    const isConsole = this instanceof winston.transports.Console;
    
    if (isConsole) {
        const color = level === 'error' ? colors.brightRed 
                   : level === 'warn' ? colors.yellow 
                   : colors.brightGreen;
        return `${colors.brightMagenta}${timestamp} ${colors.bold}${color}${icon}${moduleInfo}${processInfo}: ${message}${colors.reset}`;
    }
    
    return `${timestamp} ${icon}${moduleInfo}${processInfo}: ${message}`;
});

// 创建日志实例
const logger = winston.createLogger({
    // 从配置文件读取日志级别，默认为info
    level: process.env.LOG_LEVEL || 'info',
    format: winston.format.combine(
        winston.format.timestamp({
            format: 'YYYY-MM-DD HH:mm:ss'
        }),
        logFormat
    ),
    transports: [
        // 控制台输出
        new winston.transports.Console(),
        // 错误日志文件
        new winston.transports.DailyRotateFile({
            filename: path.join(logDir, 'error-%DATE%.log'),
            datePattern: 'YYYY-MM-DD',
            level: 'error',
            maxFiles: '14d',
            createSymlink: true,
            symlinkName: 'error-latest.log'
        }),
        // 常规日志文件
        new winston.transports.DailyRotateFile({
            filename: path.join(logDir, 'combined-%DATE%.log'),
            datePattern: 'YYYY-MM-DD',
            maxFiles: '14d',
            createSymlink: true,
            symlinkName: 'latest.log'
        })
    ]
});

// 扩展日志方法
const enhancedLogger = {
    error: (message, module = '') => logger.error(message, { module }),
    warn: (message, module = '') => logger.warn(message, { module }),
    info: (message, module = '') => logger.info(message, { module }),
    http: (message, module = '') => logger.http(message, { module }),
    verbose: (message, module = '') => logger.verbose(message, { module }),
    debug: (message, module = '') => logger.debug(message, { module }),
    
    setLevel: (level) => {
        logger.level = level;
        return logger.level;
    },
    
    getLevel: () => logger.level,
    
    // 添加清理旧日志文件的方法
    cleanOldLogs: async (days = 14) => {
        const oldDate = new Date();
        oldDate.setDate(oldDate.getDate() - days);
        
        try {
            const files = await fs.promises.readdir(logDir);
            for (const file of files) {
                if (file.endsWith('.log')) {
                    const filePath = path.join(logDir, file);
                    const stats = await fs.promises.stat(filePath);
                    if (stats.mtime < oldDate) {
                        await fs.promises.unlink(filePath);
                        logger.info(`Deleted old log file: ${file}`, 'Logger');
                    }
                }
            }
        } catch (error) {
            logger.error(`Failed to clean old logs: ${error.message}`, 'Logger');
        }
    }
};

module.exports = enhancedLogger; 