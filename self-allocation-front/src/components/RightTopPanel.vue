...
const expandedDevices = computed(() => {
  ...
  return props.devices.map((device, index) => ({
    ...device,
    uniqueId: `edge1-${device.name}-${index}`
  }))
})
...
watch(
  expandedDevices,
  (newDevices) => {
    console.log('RightTopPanel 扩充后的 edge1 设备数据:', newDevices)
  },
...
watch(
  ...
  (newCount) => {
    if (newCount.data) {
      console.log('RightTopPanel收到调度计数:', {
...
const updateCharts = () => {
  ...
  metrics.forEach((metric, index) => {
    const chart = charts.value[`edge1-chart${index + 1}`]
    if (chart) {
      chart.setOption({
        series: [
          {
...
onMounted(async () => {
  ...
  const chartIds = ['edge1-chart1', 'edge1-chart2', 'edge1-chart3']
  const colors = ['#ffd966', '#f4b183', '#d9f36d']
...
const statusText = computed(() => {
  if (!props.scheduleCount.data?.edge1) {
    return {
      AI: '0',
      Rendering: '0',
      Simulation: '0'
  ...
  const edge1Data = props.scheduleCount.data.edge1
  return {
    AI: edge1Data.ai?.toString() || '0',
    Rendering: edge1Data.rendering?.toString() || '0',
    Simulation: edge1Data.simulation?.toString() || '0'
  }
})
</script>
...
<template>
  <div class="right-top-panel">
    <div class="topShow">
      <div class="title-cloud">Edge1</div>
      <div class="usage">
        <div class="charts-container">
          <div class="chart-item">
            <div class="chart-title">CPU</div>
            <div id="edge1-chart1" class="chart"></div>
          </div>
          <div class="chart-item">
            <div class="chart-title">GPU</div>
            <div id="edge1-chart2" class="chart"></div>
          </div>
          <div class="chart-item">
            <div class="chart-title">RAM</div>
            <div id="edge1-chart3" class="chart"></div>
          </div>
        </div>
    ...
    <div class="bottomShow">
      <EdgeNodeStatus
        ...
        :device-data="device"
      />
      <div class="cloud-distance">50KM</div>
    </div>
  </div>
</template>
